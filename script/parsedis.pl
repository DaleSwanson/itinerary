...
my $inputfile = "distance.output.csv";
my $outputfile = "distances.".time.".csv";
...
{#assume each fileline is good
	$to=0;
	my @values = split(';', $fileline); #an array of distance in meters from one state to every other
	foreach my $thisstate (@values)
	{#print each value out to file
		if ($thisstate =~ m/\d+/)
		{
			$thisstate /= 1609.34; #meters to miles
